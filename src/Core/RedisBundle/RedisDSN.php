...
class Redis {
     ...
     * )
     */
    public function getNodeConnection(array $node) {
        $hostKey = "node://{$node['host']}:{$node['port']}.{$node['timeout']}/{$node['options']['database']}";
        if (isset($this->connections[$hostKey])) {
            return $this->connections[$hostKey];
        } else {
            $Redis = new \Redis;
            $connectFunction = ($node['options']['persistent']) ? 'pconnect' : 'connect';
            if ($Redis->$connectFunction($node['host'], (int) $node['port'], (float) $node['timeout'], $hostKey)) {
                if ($node['options']['prefix']) {
                    if (!$Redis->setOption(\Redis::OPT_PREFIX, $node['options']['prefix'])) {
                        throw new RedisException("Could not set prefix for Redis node");
                    }
                }

                if ($node['options']['database']) {
                    if (!$Redis->select($node['options']['database'])) {
